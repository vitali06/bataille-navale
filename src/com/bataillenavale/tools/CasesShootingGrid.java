 ...
 */
public class CasesShootingGrid {
    ...
    private List<ShootingGridGDX> m_actors = new LinkedList<>();
     ...
     * @param mode
     */
    public CasesShootingGrid(int x, int y, String mode) {
     ...
     */
    public CasesShootingGrid(int x, int y, int width, int height) {
        this.m_positions.put(x, y);
        this.m_aWidth = width;
    ...
    private void activeCase() {
        for (Entry<String, ShootingGridGDX> entry : Singleton.getGraphic().getShootingGrid().entrySet()) {
            if (entry.getValue().name.startsWith("ShootingCase") 
                    || entry.getValue().name.startsWith("ShootingSelect") 
                    || entry.getValue().name.startsWith("ShootingUse") 
                    || entry.getValue().name.startsWith("ShootingCheck")) {
     ...
     */
    private void selectCase(List<ShootingGridGDX> a) {
        for (ShootingGridGDX item : a) {
            if (item.name.startsWith("ShootingCase") && hit(item)) {
                this.m_active = true;
                if (!Singleton.getGraphic().getVisible("ShootingUse" + item.name.substring(item.name.length() - 2)) && !Singleton.getGraphic().getVisible("Check" + item.name.substring(item.name.length() - 2)) && !Singleton.getGraphic().getVisible("Select" + item.name.substring(item.name.length() - 2))) {
                    Singleton.getGraphic().setVisible("ShootingCase" + item.name.substring(item.name.length() - 2), false);
                    System.out.println(item.name.substring(item.name.length() - 2));
                    Singleton.getGraphic().setVisible("ShootingSelect" + item.name.substring(item.name.length() - 2), true);
                }
            } else if (item.name.startsWith("ShootingCase") && !hit(item)) {
                if (!Singleton.getGraphic().getVisible("ShootingUse" + item.name.substring(item.name.length() - 2)) && !Singleton.getGraphic().getVisible("Check" + item.name.substring(item.name.length() - 2))) {
                    Singleton.getGraphic().setVisible(item.name, true);
                    Singleton.getGraphic().setVisible("ShootingSelect" + item.name.substring(item.name.length() - 2), false);
                }
            }
        }
    }
     ...
     */
    private void useCase(List<ShootingGridGDX> a) {
        for (ShootingGridGDX item : a) {
            if (item.name.startsWith("ShootingCase") && hit(item)) {                
                if (!Singleton.getGraphic().getVisible("ShootingUse" + item.name.substring(item.name.length() - 2)) && !Singleton.getGraphic().getVisible("Check" + item.name.substring(item.name.length() - 2))) {
                    Singleton.getGraphic().setVisible("ShootingCase" + item.name.substring(item.name.length() - 2), false);
                    Singleton.getGraphic().setVisible("ShootingSelect" + item.name.substring(item.name.length() - 2), false);
                    Singleton.getGraphic().setVisible("ShootingUse" + item.name.substring(item.name.length() - 2), true);
                }
            }
        }
    }
     ...
     * @param a List of Actor Case
     */
    private void checkCase(List<ShootingGridGDX> a) {
        for (ShootingGridGDX item : a) {
            if (item.name.startsWith("ShootingCase") && hit(item)) {
                if (!Singleton.getGraphic().getVisible("ShootingUse" + item.name.substring(item.name.length() - 2)) && !Singleton.getGraphic().getVisible("Check" + item.name.substring(item.name.length() - 2))) {
                    Singleton.getGraphic().setVisible("ShootingCase" + item.name.substring(item.name.length() - 2), false);
                    Singleton.getGraphic().setVisible("ShootingSelect" + item.name.substring(item.name.length() - 2), false);
                    Singleton.getGraphic().setVisible("ShootingCheck" + item.name.substring(item.name.length() - 2), true);
                }
            }
        }
    }
     ...
     * @return True if HIT
     */
    private boolean hit(ShootingGridGDX a) {
     ...
     */
    public int getX() {
        ShootingGridGDX result = null;
        int i = 0;
        for (ShootingGridGDX item : this.m_actors) {
            if (item.name.startsWith("ShootingCase")) {
                if (i < 1) {
                    result = item;
                    i++;
    ...
    public int getY() {
        int y = 0;
        for (ShootingGridGDX a : this.m_actors){
            if (hit(a)){
                y = (int)a.y;
            }
        }
     ...
     * @return Actor case
     */
    private ShootingGridGDX lastCase() {
        ShootingGridGDX result = null;
        int i = 0;
        for (ShootingGridGDX item : this.m_actors) {
            if (item.name.startsWith("ShootingCase")) {
